...
const generateToken = async (res, user_id, username, is_writer) => {
  ...
  res.cookie(
    ...
    { 
      ...
      secure: true, // TODO switch to true with https
      httpOnly: true,
    },
  );
};

exports.assignUsernameWriter = async (req, res, next) => {
  try {
    ...
    res.locals.username = identity.username;
    res.locals.is_writer = identity.is_writer;
    next();
  } catch (err) {
    next(err);
  }
...
exports.loginPOST = async (req, res, next) => {
  try {
    ...
    if (result.length === 0) {
      return res.render('login', {login_error: 'Invalid credentials'})
    }
    const user = result[0];
